...
import (
	...
	handlers "github.com/rodkevich/ts/photo/internal/blueprints/grpc"
	"github.com/rodkevich/ts/photo/internal/controllers"
	"github.com/rodkevich/ts/photo/internal/repositories/postgres"
	"github.com/rodkevich/ts/photo/pkg/logger"
	pb "github.com/rodkevich/ts/photo/proto/photo/v1"
)
...
const (
	...
	maxHeaderBytes     = 1 << 20
	photoCachePrefix   = "photos:"
	photoCacheDuration = time.Minute * 15
)
...
func (s *Server) Run() error {
	...
	photoDB := postgres.NewPhotoPG(s.pgConnection)
	photoController := controllers.NewPhotoController(s.logger, photoDB)
	photoService := handlers.NewPhotoGrpcService(s.logger, photoController)
	pb.RegisterPhotoServiceServer(serverGRPC, photoService)
